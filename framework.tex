...
that a system meets some requirement in a dynamic environment.  In
this section we describe a typical control system for managing compute
resources and then describe how to generalize it for our purposes.  We
restrict ourselves to the set of controllers that manage performance
...
\end{equation}
Here, the performance observed at time $perf(t)$ is predicted as some
constant $k$ times the clockspeed applied at the previous time step,
...
behavior of the next time step as a function of one variable setting
from the previous time step.  Using the relationship of
\eqnref{clock}, we can synthesize a simple deadbeat controller that is
provably convergent to the desired performance:
\begin{equationarray}
...
\cite{METE,others}.  Either way, the control system suffers a similar
tension as our learners -- we could set the value of $k$ to be
...
models to individual applications.  We do this by combining learning
and control as described in the next section.


\TODO{I am not sure it is clear how control addresses dynamics.  What
  can we add?}
...
\begin{figure}
\includegraphics[width=\columnwidth]{figs/ControlLearning.png}
...
standard control theoretic formal guarantees.  By measuring base speed
online while the application runs, we can tune the control to a
specific application.  We note that using this definition of base
...
\begin{figure}
\includegraphics[width=\columnwidth]{figs/SpeedupHashTable.png}
...
example in \figuref{pht} has a resolution of .1.  Each pointer in the
