...
In general, machine learning models are predictive in nature.
They take observations of some phenomena and turn them into a model to
predict future outcomes. In this work, we use a hierarchical Bayesian model (HBM) to turn observations of applications' performance and power into predictions
...
performance and power tradeoff spaces for two example applications
...
with darker colors representing larger performance/power. The presence of local minima and maxima mean that simple gradient
...
applications \texttt{Kmeans} and \texttt{lavamd} against
...
non-smoothness is the reason why parametric models based on clockspeed/Cores of these curves do
...
\SYSTEM{} uses an HBM based on LEO \cite{LEO}, a learning system build to
estimate application performance and power for server systems.  This HBM based model runs on a remote server (originally in LEO it ran natively on the system to be optimized). 

We compare \SYSTEM{} to an \emph{online model}, which only uses the information on performance of the given application at different configurations. In order for such model to be predictive, it must be parametric in terms of those configuration values (cores/clockspeed) since no other information is available. The model would require certain number of samples to estimate the model parameters with low error. Unfortunately, the model might still falter if the model parameterization isn't correct, \eg{} if the model is misspecified and defined to be linear in cores but unlike compute-bound applications, the given application has limited parallelization and does not improve with more cores.

We also compare \SYSTEM{} to a purely \emph{offline model} which only uses information from previously seen applications but lacks any knowledge of the application in hand. This general model will capture the trend, \eg when most applications
should transition, but it might miss the key
...
The HBM (illustrated in \figref{HBM}) is a superior combination of the above discussed approaches (\emph{online} and \emph{offline}). The
model benefits from both, the observations of the current application
and the previously observed applications.  The model relies on accurately learning the correlations between different configuration based of the data from previously seen applications. Since, dimension of the model's parameter, the correlation matrix scales with the number of configurations the model is non-parametric. 
Unlike an \emph{offline} scenario, here each application has its own
...
previously made of applications . We would generate a model which most agrees with the learnt correlations and the new samples in hand. Rather than over-generalizing, the
...
intuition behind our solution is illustrated in \figref{convexhull}.
...
The HBM stores the lower convex hull in a \emph{performance hash
  table} (PHT).  The PHT is illustrated in \figref{pht}.  It consists
...
By solving \eqnsref{s1}{s2}, the translator has turned the
