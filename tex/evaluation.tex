...
deadline misses and energy over optimal for all baselines.  We note
...
average of 20\%, which is only \TODO{XXX} more than optimal.  It also
...
Before the second application starts, both HBM and \SYSTEM{} do a good
job of tracking the target and keeping energy low.  In contrast,
Adaptive-Control produces oscillating performance (and thus power)
because it has a bad model of \texttt{bodytrack}'s LITTLE core
...
\end{figure}
We demonstrate that \SYSTEM{} reacts to input variations.
\figref{fig:x264-phase-change} shows the \texttt{x264} video encoder
application with 2 different phases, where the phase change occurs at
the $500^{th}$ frame. The first scene is difficult and the second one
is significantly easier. In the first phase, \SYSTEM{} and the HBM are
closer to the desired performance (1 in the figure) compared to
Adaptive-Control.  Additionally, these two approaches consume less
energy than Adaptive-control indicating that Adaptive-Control is
operating at a configuration not on the Pareto frontier of power and
performance.  When the input changes, \SYSTEM{} and HBM both
algorithms are still able to meet the performance target with far less
fluctuations as compared to POET.  But, \SYSTEM{} provides better
energy savings as compared to HBM. \TODO{I think we need a better
  example here.  This example has the learner reacting better to a
  phase change than the controller, which contradicts what we said
  before.  The problem is that the learner isn't really a learner, but
  a controller itself.}
...
\TODO{Add the LAVAMD contour plot?}  LAVAMD has one of the most
complicated responses to resource usage on our system, as shown in
\figref{fig:lavamd_contour}. \secref{guarantees} presents an
analytical argument that tuning the controller to model variance and
confidence interval will prevent oscillation and provide probabilistic
control theoretic guarantees despite using noisy, learned models to
control such complicated behavior.  We now demonstrate this
...
accuracy for the Online, Nuclear, and HBM learners.  The accuracy is
simply how close the model is to ground truth (found through
exhaustive exploration), with 1 meaning the model perfectly reproduced
the real performance or power.  This number is significant, because
the smaller the number of samples, the faster the controller can
switch from a general model to the HBM's application specific model.

\figref{fig:sensitivity} compares \SYSTEM{}'s accuracy to Online and
NUCLEAR for learning both performance (top) and power (bottom).  The
figure shows sample size on the x-axis and accuracy on the y-axis.
\SYSTEM{}'s HBM initially performs as well as the Offline approach and
as sample size increases, the accuracy uniformly improves and reaches
greater than 90\% around 20 samples. The online approach needs at
least 7 samples to begin model building.  As we get more samples the
accuracy for the online model improves but still does not meet
\SYSTEM{}'s accuracy for the same number of samples. Nuclear is very
noisy as small numbers of samples, but once the number of samples
reaches about 50, it is competitive with HBM.  These results not only
demonstrate the sensitivity to sample size, they show why
\SYSTEM{}-HBM achieves better results than the other learning
mechanisms.
