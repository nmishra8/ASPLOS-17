...
\begin{figure}[htp!]
\centering
  \input{img/single-app-summary2.tex}
   \vskip -.5em
...
\figsref{fig:single-perf}{fig:single-energy} shows the results for the
50, 70, and 90\% targets\footnote{We would happily provide that data
  anonymously through the PC chair if a reviewer finds it important to
  the evaluation}, others are similar but elided for space.  The
...
consumption than the baseline algorithms.  \figref{fig:single-sum}
represents the summary results as an average error across all targets for single application scenario.
...
\begin{figure}[htp!]
\centering
  \input{img/multi-app-summary2.tex}
   \vskip -.5em
...
\figref{fig:multi-sum} represents the summary results as an average error across all targets for multiple application scenario.
