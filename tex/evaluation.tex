...
We perform our experiments on four ODROID-XU3 devices which are small
mobile devices running Ubuntu 14.04. The ODROID boards have Samsung
Exynos 5 Octa processors based on the ARM big.LITTLE architecture.
Each processor has 19 speed settings for its 4 big cores and 13 for 4
LITTLE cores.  Each board has an on-board power meter updated at 1/4
\ms intervals.  Each resource configuration (combination of big/LITTLE
cores and clock-speeds) has a different performance and power, which
is, itself, application-dependent.
...
\begin{figure*}[t]
  \input{img/single-app-performance.tex}
   \vskip -1em
  \caption{Comparison of application performance error for single application.}
...
\begin{figure*}[t]
  \input{img/single-app-energy.tex}
   \vskip -1em
  \caption{Comparison of application energy consumption for single application.}
...
\begin{figure*}[htp!]
  \input{img/multi-app-performance.tex}
   \vskip -1em
  \caption{Comparison of application performance error for multiple applications.}
...
\begin{figure*}[t]
  \input{img/multi-app-energy.tex}
   \vskip -1em
  \caption{Comparison of application energy consumption for multiple applications.}
...
\begin{figure}[t]
  \input{img/x264-phases2.tex}
   \vskip -1em
  \caption{Time series of \texttt{X264} with phase change caused by different input.}
