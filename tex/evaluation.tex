...
with:
\begin{enumerate}
\item \textit{Race-to-idle}: \TODO{Add description and citation}
\item \textit{PID-Control}: \TODO{Add description and citation}
...
\item \textit{Offline}: does not observe any values for the current
  ...
  estimate power and performance as a linear regression. \PUNT{We use
    ...
    \emph{Pace-to-idle (P2I)} heuristic.}
\item \textit{HBM}: is a hierarchical Bayesian learner \cite{LEO}.
...
\item \textit{CALOREE-NoPole}: is CALOREE with pole set to 0, which we
...
offline: 3.6\%, LEO: 8.0\%, POET: 10.4\%, CALOREE-NoPole: 6.4\% and
...
21\% more energy than optimal, offline: 15\%, HBM: 14\%, POET: 24\%,
CALOREE-NoPole: 9.5\%, and \SYSTEM{}: 4.1\%.  }
\SYSTEM{} provides a
...
complementary: HBM produces accurate models while control both can
