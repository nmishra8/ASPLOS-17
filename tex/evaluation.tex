...
board has an on-board power meter updated at 1/4 \ms intervals
capturing core, GPU, memory, and flash drive power.  
...
\SYSTEM{} under each constraint. We quantify performance reliability
by measuring the number of deadlines that were missed for each
...
\item \textit{CALOREE-NoPole}: uses the HBM learner, but sets the pole
  to 0, showing the importance of incorporating the learned variance
  into control. All other versions of \SYSTEM{} set the pole according
  to \secref{guarantees}.
...
\figsref{single-perf}{single-energy} show the detailed results
...
\figsref{multi-perf}{multi-energy} shows the detailed results.  The
