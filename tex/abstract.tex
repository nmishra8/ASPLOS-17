\begin{abstract}
  ...
  complex resource interaction. We therefore propose \SYSTEM{}, a
  parameter-free combination of learning and control that
  automatically adjusts resource usage to meet performance
  requirements in complex, dynamic environments.  \SYSTEM{} includes
  (1) a control abstraction where key parameters are automatically
  estimated by a noisy learning mechanism, (2) an efficient
  implementation that applies the learned models in constant time, and
  (3) formal guarantees that the system converges to the desired
  performance without prior knowledge of application behavior. We
  implement \SYSTEM{} and test its ability to meet application
  performance requirements on heterogeneous ARM big.LITTLE
