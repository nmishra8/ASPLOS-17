\begin{abstract}
  ...
  Embedded and mobile devices must process streaming data from
  sensors.  Control theoretic resource managers adjust resource usage
  to meet streaming applications' performance requirements with
  minimal energy. Their designs, however, require \emph{a priori}
  models to map system resource allocation to application performance
  and energy.  Thus, a controller designed for one application and
  system must be redesigned to work in a new deployment.  We propose
  CALOREE to achieve control theoretic design benefits -- \ie formal,
  ...
  control system (GCS) that runs on a local device with a hierarchical
  Bayesian model (HBM) that runs on a remote server. The HBM
  aggregates data from multiple applications and devices to produce
  highly accurate models that are sent to the GCS, customizing control
  to the particular application and device.  We extend standard
  control theoretic formal analysis to show that CALOREE provides
  probabilistic convergence guarantees despite having no prior model
  ...
  enter the system and compete for resources).  Compared to
  state-of-the-art learning and control techniques, CALOREE's
  ...
  Additionally, CALOREE provides the lowest energy with average
  savings anywhere from 8-47\%.
