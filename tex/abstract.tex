\begin{abstract}
  ...
  energy based on the resources allocated to it -- a controller
  designed for one application and system must be redesigned for a new
  deployment.  We propose CALOREE to achieve control theoretic
  benefits -- \ie{} formal, bounded convergence to required performance
  in dynamic environments -- without \emph{a priori} models.  CALOREE
  combines a generalized control system (GCS) \PUNT{that runs on a
    local device} with a hierarchical Bayesian model (HBM)\PUNT{that
    runs on a remote server}. The HBM runs on a remote server and
  aggregates data from multiple applications and devices to produce a
  highly accurate model that it sends to the GCS to customize control
  to the local device and application. We extend standard control
