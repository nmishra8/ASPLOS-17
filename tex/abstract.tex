\begin{abstract}
  ...
  We propose CALOREE to achieve the benefits of control theoretic
  design -- \ie formal, bounded convergence to required performance in
  dynamic environments -- without \emph{a priori} models.  CALOREE
  combines a generalized control system (GCS) that runs on a local
  device with a hierarchical Bayesian model (HBM) that runs on a
  ...
  GCS, customizing control to the particular application and device.
  We extend standard control theoretic formal analysis to show that
  ...
  performance and energy compared to state-of-the-art learning and
  control techniques.  We find that CALOREE's unique combination of
  control and learning consistently provides the most reliable
  performance, with a worst case error (in the multi-app case) between
  desired and delivered performance of only 12\% compared to 70-80\%
  for prior approaches.  Additionally, CALOREE consistently provides
  the lowest energy with average savings anywhere from 8-47\%.
