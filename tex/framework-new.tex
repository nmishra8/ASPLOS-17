...
management into separate learning and control tasks and defining an
interface between them.  When a new application enters the system with
a performance requirement, an adaptive control system allocates
resources using generic predictions and records performance and power.
The recorded values are sent to a learner, which predicts the
application's performance and power in all other resource
configurations. The learner extracts those that are Pareto-optimal and
packages them in a special data structure: the performance hash table
(PHT).  The PHT and the estimated variance are sent to the controller,
which sets its pole and selects an energy minimal resource
configuration with formal guarantees of convergence to the desired
performance.  \SYSTEM{}'s only user-specified parameter is the
...
device type (message 1, \figref{fig:timeline}).  The learner
...
\SYSTEM{} must reduce the error between the target performance and the goal: $ error(t) = goal - perf(t)$.
...
example in \figref{fig:pht} has a resolution of $0.1$.  Each pointer
...
\PUNT{When selecting a learning framework we must find a tradeoff between
...
applications---the learner in \figref{fig:timeline} can simply
...
% % application.
% \subsubsection{Offline Predictors}
