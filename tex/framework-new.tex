...
and records performance and power.  The recorded values are sent to a
learner, which predicts the application's performance and power in all
...
to the desired performance.  \SYSTEM{}'s only user-specified parameter
...
\SYSTEM{} must eliminate the error between the target performance and the goal: $ error(t) = goal - perf(t)$.
